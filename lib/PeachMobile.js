...
export default class PeachMobileComponent extends Component {
    ...
    submitTransaction(transaction) {
        ...
        const submitTransactionCompletionHandler = (checkoutTransaction) => {
            ...
            return waitForAsynchronousPaymentCallback(500, 0.3).then(() => {
        ...
        if (!transaction) {
            return PeachMobileComponent.createTransaction(checkoutID, paymentBrand, cardHolder, cardNumber, cardExpiryMonth, cardExpiryYear, cardCVV)
                .then(createdTransaction => {
                    return submitTransaction(createdTransaction).then(submitTransactionCompletionHandler);
                });
        }
